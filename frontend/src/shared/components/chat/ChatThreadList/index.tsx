// src/shared/components/chat/ChatThreadList/index.tsx
...
interface ChatThreadListProps {
  ...
  onDeleteChatStart?: () => void;
}

function ChatThreadList({ threads, onThreadClick, onDeleteChatStart }: ChatThreadListProps) {
  const dispatch = useDispatch();
  const [clearThread] = useClearThreadMutation();

  const handleClearThread = (threadId: number, otherUserName: string, event: MouseEvent) => {
  ...
  return (
    <div className={styles.threadList}>
      {threads.map((thread) => (
        <div key={thread.id} className={styles.threadItem} onClick={() => onThreadClick(thread.id)}>
          <Group gap='sm' wrap='nowrap'>
            <Avatar src={thread.other_user?.image_url ?? null} radius='xl' size='sm' />
