...
function MessageList({
  ...
  const handleScroll = () => {
    ...
    const isAtBottom =
      viewport.current.scrollHeight - viewport.current.scrollTop - viewport.current.clientHeight <
      10;
  ...
  const formatUserName = (user: MessageUser | null | undefined): string => {
    if (!user) return 'Unknown User';
    return (
      user.full_name || `${user.first_name || ''} ${user.last_name || ''}`.trim() || 'Anonymous'
    );
  };
    ...
    >
      <div className={styles.messagesList}>
        ...
        {messages.map((message) => {
          ...
          const messageUser = message.user as MessageUser | undefined;
