// src/shared/components/chat/MobileChatWindow/index.tsx
...
/** Message type from useSocketMessages hook */
 ...
 */
function MobileChatWindow({ threadId, onClose }: MobileChatWindowProps) {
  ...
  // Typing indicator hook - use 0 as fallback when user is not logged in
  ...
  const { handleKeyDown, handleSendClick, handleFocus, handleBlur } = useMobileInputHandler(
    handleSendMessage,
    messageInput,
    !!otherUser,
  );
      ...
      >
        ...
        {messages.map((message: SocketMessage) => (
          <ChatMessage
            key={message.id}
            message={{
              id: typeof message.id === 'string' ? parseInt(message.id, 10) || 0 : message.id,
