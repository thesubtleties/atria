...
const AVATAR_API_URL = import.meta.env.VITE_AVATAR_API_URL || 'http://localhost:5001';
...
export const EditAvatarModal = ({ opened, onClose, onSave, currentUrl }: EditAvatarModalProps) => {
  ...
  const parseAvatarUrl = (url: string | null | undefined): AvatarOptions => {
    console.log('Parsing avatar URL:', url);

    if (!url || !url.includes('avataaars')) {
      console.log('Using default avatar options (no URL or not avataaars)');
      return { ...DEFAULT_AVATAR_OPTIONS, seed: 'user' };
    }
    ...
    try {
      ...
      const parsed: AvatarOptions = {
        top: params.get('top') || DEFAULT_AVATAR_OPTIONS.top,
        // Check both old (topColor) and new (hairColor) parameter names
        topColor:
          params.get('hairColor') || params.get('topColor') || DEFAULT_AVATAR_OPTIONS.topColor,
        facialHair: params.get('facialHair') || DEFAULT_AVATAR_OPTIONS.facialHair,
        facialHairColor: params.get('facialHairColor') || DEFAULT_AVATAR_OPTIONS.facialHairColor,
        ...
        clothesColor: params.get('clothesColor') || DEFAULT_AVATAR_OPTIONS.clothesColor,
        accessories: params.get('accessories') || DEFAULT_AVATAR_OPTIONS.accessories,
        accessoriesColor: params.get('accessoriesColor') || DEFAULT_AVATAR_OPTIONS.accessoriesColor,
        hatColor: params.get('hatColor') || DEFAULT_AVATAR_OPTIONS.hatColor,
  ...
  const [options, setOptions] = useState<AvatarOptions>(() => parseAvatarUrl(currentUrl));
  const [previewUrl, setPreviewUrl] = useState('');

  // Build avatar URL from options
  const buildAvatarUrl = (opts: AvatarOptions): string => {
  ...
  useEffect(() => {
    ...
    console.log('Updating preview with URL:', newUrl);
    setPreviewUrl(newUrl);
  }, [options]);

  const handleOptionChange = (field: keyof AvatarOptions, value: string | null) => {
    console.log(`Option changed: ${field} = ${value}`);
    if (value !== null) {
      setOptions((prev) => ({
        ...prev,
        [field]: value,
