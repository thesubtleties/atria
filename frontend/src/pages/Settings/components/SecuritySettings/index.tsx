...
type PasswordStrength = {
  strength: number;
  checks: Partial<PasswordChecks>;
};

type PasswordChangeError = ApiError & {
...
const SecuritySettings = () => {
  ...
  const handleSubmit = async (values: SecuritySettingsFormData) => {
    try {
      ...
      const error = err as PasswordChangeError;
      // Handle field errors
      if (error.fieldErrors) {
        Object.entries(error.fieldErrors).forEach(([field, errors]) => {
          // Map backend field names to form field names
          const fieldMap: Record<string, keyof SecuritySettingsFormData> = {
  ...
  return (
    <Stack gap='lg'>
      ...
      <Card className={cn(styles.passwordCard)}>
        <Group mb='md' justify='space-between' align='flex-start'>
          <div>
            <Text fw={500} className={cn(styles.cardTitle)}>
      ...
      {/* Security Tips Card */}
      <Card className={cn(styles.tipsCard)}>
        <Text fw={500} mb='md' className={cn(styles.cardTitle)}>
