...
import type { DashboardNewsItem } from '../index';
...
export const NewsSection = ({ news }: NewsSectionProps) => {
  const getTimeDifference = (date: string): string => {
    ...
    const diffInDays = Math.floor((now.getTime() - newsDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const getDotClass = (date: string, isNew: boolean): string => {
    const now = new Date();
    const newsDate = new Date(date);
    const diffInDays = Math.floor((now.getTime() - newsDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const renderDescription = (description: string, link?: string): ReactNode => {
