...
import { IconDots, IconEdit, IconTrash, IconMessages, IconMessageCircle } from '@tabler/icons-react';
import styles from './styles.module.css';

const MobileCard = ({ room, color, onEdit, onViewChat, onToggle, onDelete }) => {
  // Determine icon color based on room type
  const iconColor = color === 'blue' ? '#3B82F6' : 
                    color === 'violet' ? '#8B5CF6' : 
                    '#14B8A6';
  ...
  const getBorderColor = () => {
    ...
    return color === 'blue' ? 'rgba(59, 130, 246, 0.5)' : 
