...
const SponsorCard = ({ id, sponsor, tierId, index, onEdit }: SponsorCardProps) => {
  ...
  const handleDelete = () => {
    openConfirmationModal({
      ...
      onConfirm: async () => {
        try {
          await deleteSponsor({ sponsorId: sponsor.id }).unwrap();
          notifications.show({
            title: 'Success',
  ...
  const handleToggleActive = async () => {
    try {
      await toggleActive({ sponsorId: sponsor.id }).unwrap();
    } catch (error) {
      notifications.show({
        title: 'Error',
        message:
  ...
  const handleToggleFeatured = async () => {
    try {
      await toggleFeatured({ sponsorId: sponsor.id }).unwrap();
    } catch (error) {
      notifications.show({
        title: 'Error',
        message:
