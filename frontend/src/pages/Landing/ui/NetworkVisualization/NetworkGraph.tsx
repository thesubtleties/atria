...
const NetworkGraph = ({
  ...
  useGSAP(
    () => {
      const nodeElements = gsap.utils.toArray<HTMLElement>('.network-node');
      const connectionElements = gsap.utils.toArray<HTMLElement>('.network-connection');
  ...
  return (
    <svg ref={svgRef} viewBox={viewBox} className={`${styles.networkSvg} ${className}`}>
      {/* Connections */}
