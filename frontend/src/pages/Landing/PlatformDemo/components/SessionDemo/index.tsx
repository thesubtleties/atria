...
const chatMessages: { chat: ChatMessage[]; backstage: ChatMessage[] } = {
  chat: [
    {
      id: 1,
      user: 'Sarah M.',
...
type SessionDemoProps = {
  isFirefox: boolean;
};

export const SessionDemo = ({ isFirefox }: SessionDemoProps) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const hasAnimatedRef = useRef(false);
  const [activeTab, setActiveTab] = useState<'chat' | 'backstage'>('chat');
