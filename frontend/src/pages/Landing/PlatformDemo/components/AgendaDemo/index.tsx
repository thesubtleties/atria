...
type AgendaDemoProps = {
  isFirefox: boolean;
};

export const AgendaDemo = ({ isFirefox }: AgendaDemoProps) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const hasAnimatedRef = useRef(false);
  const expandedCardRef = useRef<number | null>(null);
  ...
  const handleMouseEnter = (e: MouseEvent<HTMLDivElement>, sessionIndex: number) => {
    ...
    if (rowCards.length > 1) {
      rowCards.forEach((rowCard) => {
        if (rowCard !== card) {
          ...
          if (otherSpeakers && styles.hiddenSpacer) {
          ...
          <div className={styles.sessionRow}>
            ...
            >
              <div className={styles.typeTag}>
                <span
                  className={`${styles.sessionType} ${styles[agendaSessions[0].type.toLowerCase()]}`}
                >
              ...
              <div className={styles.content}>
                ...
                <div className={styles.meta}>
                  <span className={styles.time}>
                    {agendaSessions[0].time} - {agendaSessions[0].endTime}
                  </span>
                </div>
        ...
        <div className={styles.sessionRow}>
          {agendaSessions.slice(1).map((session, index) => (
            ...
            >
              ...
              <div className={styles.speakers}>
                <div className={styles.speakerLabel}>MODERATOR</div>
                <div className={styles.speakerInfo}>
                  <img
                    className={styles.speakerAvatar}
